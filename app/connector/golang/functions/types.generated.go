...
func (j *CheckClearanceArguments) FromValue(input map[string]any) error {
	var err error
	j.UserId, err = utils.GetString(input, "userId")
	if err != nil {
		return err
	}
	return nil
...
func (j CheckClearanceResult) ToMap() map[string]any {
	r := make(map[string]any)
	r["disciplineClearance"] = j.DisciplineClearance
	r["dormitaryClearance"] = j.DormitaryClearance
	r["libraryClearance"] = j.LibraryClearance
...
func (dch DataConnectorHandler) execQuery(ctx context.Context, state *types.State, request *schema.QueryRequest, queryFields schema.NestedField, rawArgs map[string]any) (any, error) {
	...
	case "checkClearance":
		...
		var args CheckClearanceArguments
		...
		rawResult, err := FunctionCheckClearance(ctx, state, &args)
...
var enumValues_FunctionName = []string{"checkClearance"}
